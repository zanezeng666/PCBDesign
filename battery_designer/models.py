...
class TerminalRegion(BaseModel):
    type: str = Field(pattern=r"^(solder_pad|hole)$")
    visual_class: str
    shape: PadShape
    center: Point
    bbox: RegionBounds
...
class BatterySpec(BaseModel):
    ...
    chemistry: str = "Li-ion/LiPo"
    cell_min_v: float = Field(gt=0, lt=10)
    cell_nominal_v: float = Field(gt=0, lt=10)
    cell_max_v: float = Field(gt=0, lt=10)

    @model_validator(mode="after")
    def ordered_voltages(self) -> "BatterySpec":
        if not self.cell_min_v < self.cell_nominal_v < self.cell_max_v:
            raise ValueError("cell voltages must satisfy min < nominal < max")
        return self
...
class ManufacturingSpec(BaseModel):
    ...
    copper_oz: float = Field(default=1.0, ge=1.0, le=1.0)
